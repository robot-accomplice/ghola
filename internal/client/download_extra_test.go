...
// redirectBlobServer returns a Streamer where the first HEAD returns a 301 to
// /final, and GET /final serves the full blob. This exercises resolveTarget →
// resolveTargetViaGet → streamLocation.
//
// Because streamLocation calls gholatransport.New (not the injected Streamer),
// we spin up a real TCP listener via net/http/httptest so the production
// transport can connect.
func redirectBlobServer(t *testing.T, data []byte) (baseURL string, stream Streamer) {
	t.Helper()
	...
	mux.HandleFunc("/blob", func(w http.ResponseWriter, r *http.Request) {
		...
		// bytes=0-0 range GET used by resolveTargetViaGet
		if r.Header.Get("Range") == "bytes=0-0" {
			// Not a redirect: signal the final URL
			w.Header().Set("Accept-Ranges", "bytes")
			w.WriteHeader(http.StatusPartialContent)
			if len(data) > 0 {
				w.Write(data[:1]) //nolint:errcheck
			}
			return
		}
		w.Header().Set("Accept-Ranges", "bytes")
	...
	// Injected Streamer (used for HEAD probe calls in resolveTarget and
	// the final download). streamLocation uses its own real transport, so
	// it will also connect to srv.
	ln := fasthttputil.NewInmemoryListener()
	fasthttpSrv := &fasthttp.Server{Handler: func(ctx *fasthttp.RequestCtx) {
		path := string(ctx.Path())
		if path == "/blob" && ctx.IsHead() {
			ctx.Response.Header.Set("Location", srv.URL+"/final")
	...
	go fasthttpSrv.Serve(ln) //nolint:errcheck
